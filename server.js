...
app.post('/api/create/pptx', async (req, res) => {
  try {
    ...
    const numSlides = Math.min(parseInt((slideCount || '12').replace(/[^0-9]/g, '')) || 12, 20);

    // Gemini dan oddiy matn sifatida olish
    const prompt = `"${topic}" mavzusida ${numSlides} ta slayd tayyorla.
Har bir slayd uchun AYNAN quyidagi formatda yoz:
...
(va hokazo ${numSlides} tagacha)

O'zbek tilida, professional uslubda. Har bir nuqta aniq va ma'lumotli bo'lsin.`;

    const aiText = await gemini(prompt);
    console.log('PPT AI text length:', aiText?.length);

    // Parse text format
    const slides = [];
    if (aiText) {
      const blocks = aiText.split(/SLAYD\s+\d+/i).filter(b => b.trim());
      for (const block of blocks) {
        ...
        for (const line of lines) {
          const t = line.trim();
          if (t.toLowerCase().startsWith('sarlavha:')) {
            title = t.replace(/sarlavha:/i, '').trim();
          } else if (t.startsWith('-') || t.startsWith('•') || t.startsWith('*')) {
            points.push(t.replace(/^[-•*]\s*/, '').trim());
          } else if (t && !title.includes(t) && t.length > 5) {
            points.push(t);
          }
        }
        
        if (title && points.length > 0) {
          slides.push({ title, points: points.slice(0, 5) });
        }
      }
    }
    ...
    if (slides.length < 3) {
      console.log('Using structured fallback for:', topic);
      const fallbackSlides = [
        { title: topic, points: [`${topic} haqida umumiy ma'lumot`, "Dolzarbligi va ahamiyati", "O'rganish maqsadi"] },
        { title: "Kirish va tarix", points: ["Mavzuning kelib chiqishi", "Rivojlanish bosqichlari", "Hozirgi holat", "Asosiy muammolar"] },
        { title: "Asosiy tushunchalar", points: ["Birinchi asosiy tushuncha", "Ikkinchi asosiy tushuncha", "Ularning o'zaro bog'liqligi"] },
        { title: "Tahlil va ma'lumotlar", points: ["Miqdoriy ko'rsatkichlar", "Sifat tahlili", "Tendentsiyalar", "Prognozlar"] },
        { title: "Muammolar va yechimlar", points: ["Mavjud muammolar", "Taklif etilayotgan yechimlar", "Amalga oshirish yo'llari"] },
        { title: "Xalqaro tajriba", points: ["Rivojlangan mamlakatlar tajribasi", "Muvaffaqiyatli misollar", "O'zbekiston uchun darslar"] },
        { title: "O'zbekistondagi holat", points: ["Hozirgi vaziyat tahlili", "Amalga oshirilgan ishlar", "Rejalashtirilgan tadbirlar"] },
        { title: "Istiqbol va rejalar", points: ["Qisqa muddatli maqsadlar (1-2 yil)", "O'rta muddatli rejalar (3-5 yil)", "Uzoq muddatli istiqbol"] },
        { title: "Tavsiyalar", points: ["Davlat tashkilotlari uchun", "Xususiy sektor uchun", "Fuqarolik jamiyati uchun"] },
        { title: "Xulosa", points: ["Asosiy natijalar va xulosalar", "Muhim topilmalar", "Keyingi qadamlar"] },
        { title: "Foydalanilgan manbalar", points: ["Ilmiy adabiyotlar", "Rasmiy statistika", "Xalqaro tashkilotlar materiallari"] },
        { title: "Savol va javoblar", points: ["Savollaringiz bormi?", "Muhokama uchun mavzular", "Bog'lanish uchun: @kompyordamm"] }
      ];
      slides.push(...fallbackSlides.slice(0, numSlides));
    }
      ...
      if (idx === 0) {
        ...
        pts.forEach((point, pi) => {
          ...
          s.addText(`▶  ${point}`, {
            ...
            fontSize: 16, color: c.text, fontFace: 'Calibri', valign: 'middle'
          });
        });

        s.addText('KompYordam', {
          x: 0, y: 6.8, w: '100%', h: 0.2,
